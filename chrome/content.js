...
(function () {
  ...
  function injectJavaScript(rule) {
    ...
    // Execute via the background service worker's chrome.scripting API, which runs in
    // the page's MAIN world and bypasses CSP — a content script's own eval can't do that.
    chrome.runtime.sendMessage({
      ...
      if (chrome.runtime.lastError) {
        ...
        console.error('[GreasyBoii] JavaScript execution failed:', response && response.error);
      }
    });
  }
  ...
  function onPickerClick(e) {
    ...
    stopElementPicker();
    chrome.runtime.sendMessage({ action: 'pickerResult', selector });
  }
